...
Dim bv As Word
Dim bvl As Byte
'Dim button_pressed1 As Bit
'Dim button_pressed2 As Bit
Dim uv_psuv As Word



'With Schottky Diodes --- Voltages x 68.33
Const ov_psu = 963
Const uv_psu = 950
Const uv_psuch = 854  '12.5v
...
'If charge = True Then  'Lower alarm threshold condition when charge is true
	uv_psuv = uv_psuch
'Else
'uv_psuv = uv_psu
'Endif
...
ASM:        clrwdt
	...
	If batvolts < ov_psu And batvolts > warn_batt Then  'Voltage OK between 11V and 14V
		...
		discharge = True
		out_ctrl = True
		If balarm = True Then alarm_on = True
		balarm = False
	Endif
